...
发布 `v0.3.0` 后，可直接运行 GitHub Container Registry 镜像：
...
docker build --platform linux/amd64 \
  -t ghcr.io/lxw112190/lw.ppocr.opencvdnn:0.3.0 .
docker compose up -d
```
...
| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `PPOCR_IMAGE` | `ghcr.io/lxw112190/lw.ppocr.opencvdnn:0.3.0` | 镜像名称和标签 |
...
After the `v0.3.0` image is published:
...
| `LW_PPOCR_FILE_LOGGING_ENABLED` | Rotating file logging switch |
| `LW_PPOCR_REQUEST_LOGGING_ENABLED` | Per-request logging switch |
| `LW_PPOCR_LOG_FILE` | Rotating log file path |
