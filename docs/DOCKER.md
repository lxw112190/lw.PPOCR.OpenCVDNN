...
发布 `v1.1.0` 标签后，可直接运行 GitHub Container Registry 正式镜像：
...
docker build --platform linux/amd64 \
  -t ghcr.io/lxw112190/lw.ppocr.opencvdnn:1.1.0 .
docker compose up -d
```
...
| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `PPOCR_IMAGE` | `ghcr.io/lxw112190/lw.ppocr.opencvdnn:1.1.0` | 镜像名称和标签 |
...
After the `v1.1.0` image is published:
