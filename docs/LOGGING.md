...
v0.4.0 separates human-readable runtime diagnostics from machine-readable OCR
access records. The design borrows Nginx's operational strengths without using
the web-oriented `combined` format.
...
- `logs/access.log`: one completion record per OCR request. The default format
  is JSON Lines: one independent JSON object per line.
...
- `logs/runtime.log`：启动、模型加载、警告、异常、崩溃以及可选的
  `request_started` 线索。
- `logs/access.log`：每个 OCR 请求完成后记录一行；默认使用 JSON Lines。
...
`access_format` accepts `jsonl` or `text`. Runtime and access file paths must be
different. Both files use independent size-based rotation with the same size
and retention settings.

`access_format` 只能为 `jsonl` 或 `text`。运行日志和访问日志必须使用不同文件；
二者分别按 `max_file_size_mb` 和 `max_files` 轮转。
