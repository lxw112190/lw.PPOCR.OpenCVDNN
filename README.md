...
> 当前版本：`v0.1.0`，公共接口处于早期验证阶段，尚未承诺 ABI 长期冻结。
...
- `enabled=false`：关闭 spdlog 运行日志。
- `request_enabled=false`：保留运行日志，但关闭逐请求日志。
- 文件日志按大小轮转。每个请求先落盘 `request_started`，完成后再记录状态、耗时和结果数量，便于定位“处理哪一个请求时退出”。
