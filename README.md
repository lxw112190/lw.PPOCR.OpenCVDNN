...
> 当前正式版本：`v1.1.0`。C ABI v1、HTTP API v1、配置 Schema v1、JSONL
...
以下结果来自 2026-08-01 的 Windows 本机测试：AMD Ryzen 7 7735H（8 核 16
线程）、16 GB 内存、Windows 10 22H2、OpenCV DNN 5.0.0 CPU。HTTP 客户端
通过回环地址使用 keep-alive 和二进制图片请求，正式采集前执行并发预热，测试期间
关闭文件日志。

| 场景 | 引擎数 / 并发 | 吞吐 | 客户端 P95 | 热身后 RSS |
| --- | ---: | ---: | ---: | ---: |
| 完整 OCR | 1 / 1 | 6.17 req/s | 177.97 ms | 约 151 MB |
| 完整 OCR | 2 / 2 | 6.98 req/s | 336.50 ms | 约 284 MB |
| 仅识别（已裁剪文字行） | 1 / 1 | 126.14 req/s | 9.39 ms | 约 154 MB |
| 仅识别（已裁剪文字行） | 2 / 2 | 172.69 req/s | 13.68 ms | 约 287 MB |
| 仅识别（已裁剪文字行） | 4 / 4 | 199.80 req/s | 24.32 ms | 约 551 MB |

1000 次多尺寸、并发 4 稳定性测试全部成功，RSS 净增长 `2.32 MB`，异常输入后
正常 OCR 可以恢复。完整 OCR 在这台机器上使用 4 个引擎会因 CPU 争用而降低吞吐，
因此默认仍建议从 `engine_instances=1` 开始；仅识别场景可以从 2 个引擎开始评估。
这些结果用于配置选型参考，不代表其他图片、CPU 或操作系统上的性能承诺。

详细环境、测试方法、完整延迟分位数、内存数据和复现命令见
